  ...
  minNumGenObjectsToPassFilter_(iConfig.getParameter<unsigned int>("minNumGenObjectsToPassFilter"))
{
  ...
  if (ifTauColl_)
  {
    produces<std::vector<T> >( "IgnoreThis" );
...
{
  std::cout << "<-------------------New GenMatchedRecoObjectProducer--------------------------->" << std::endl;
  ...
  for (typename edm::RefVector<std::vector<T> >::const_iterator iRecoObj = pRecoObjs->begin(); iRecoObj != pRecoObjs->end(); ++iRecoObj) 
...
std::cout << "selectedGenObjs->size()= " <<  selectedGenObjs.size() << std::endl;
  ...
  std::auto_ptr<std::vector<T> > SetOfTaus ( new std::vector<T> ); //Only Needed for ifTauColl_ 
...
std::cout << "ifTauColl_= " << ifTauColl_ << std::endl;
  if (ifTauColl_)
  {
    for (typename std::vector<T>::const_iterator iRecoObj = pBaseRecoObjs->begin(); iRecoObj != pBaseRecoObjs->end(); ++iRecoObj) 
    {
  ...
  for (std::vector<GenTauDecayID>::iterator iGenObj = selectedGenObjs.begin(); iGenObj != selectedGenObjs.end(); ++iGenObj) 
  {
    ...
    const T* nearestRecoObj = Common::nearestObject(visibleGenParticleRef, recoObjPtrs, nearestRecoObjKey);
    ...
    {
      genMatchedRecoObjs->push_back(edm::Ref<std::vector<T> >(pBaseRecoObjs, pRecoObjs->at(nearestRecoObjKey).key()));
      std::cout << "\tFound GenRecoMatch" << std::endl;
      if (ifTauColl_)
      {
	int iter = 0;
	edm::Ref<std::vector<T> > RecoTau = edm::Ref<std::vector<T> >(pBaseRecoObjs, pRecoObjs->at(nearestRecoObjKey).key());
        for (typename std::vector<T>::const_iterator iRecoObj = SetOfTaus->begin(); iRecoObj != SetOfTaus->end(); ++iRecoObj)
        {
...
std::cout << "\t\titer= " << iter << "  iRecoObj - SetOfTaus->begin()= " <<  iRecoObj - SetOfTaus->begin() << std::endl;
  ...
  if (ifTauColl_)
  {
    const edm::OrphanHandle<std::vector<T> > SetOfRecoTausProd = iEvent.put( SetOfTaus, "IgnoreThis"  );

    std::auto_ptr<edm::ValueMap<int> > TauMatchedMap(new edm::ValueMap<int>());
    edm::ValueMap<int>::Filler int1Filler(*TauMatchedMap);
    for (unsigned int i = 0; i< genMatchedRecoObjs_Matched.size(); ++i)
      std::cout << "genMatchedRecoObjs_Matched[" << i << "]= " << genMatchedRecoObjs_Matched[i] << std::endl;
  ...
  std::cout << "<-------------------End GenMatchedRecoObjectProducer--------------------------->" << std::endl;
