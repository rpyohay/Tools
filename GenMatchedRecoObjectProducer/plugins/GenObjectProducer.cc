...
private:
  ...
  /*save this gen object
    ...
    status 2 tau from the decay of momPDGID_
    - if primaryTauDecayType_ == GenTauDecayID::MU or primaryTauDecayType_ == GenTauDecayID::E, 
    the reco::GenParticle object saved is the charged lepton daughter of the status 2 tau that may 
    itself be a daughter of a status 23 tau from the decay of momPDGID_*/
...
bool GenObjectProducer::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (reco::GenParticleCollection::const_iterator iGenParticle = pGenParticles->begin(); 
	...
	is the status 2 tau*/
      if (((fabs(iGenParticle->pdgId()) == GenTauDecayID::TAUPDGID) && 
	  ...
	  /*if matching to gen non-taus: status whatever mother --> status whatever non-gen tau ==> 
	    ...
	    itself*/
	  ((fabs(iGenParticle->pdgId()) != GenTauDecayID::TAUPDGID) && 
	   tauDecay.hasRightMother())) aDecayProducts.push_back(tauDecay);
    }
  ...
  /*find the decay type of each a decay product if it's a tau (needed to get the visible 
    4-vector), otherwise use the 4-vector of the gen particle you're trying to select*/
  ...
  for (std::vector<GenTauDecayID>::iterator iTau = aDecayProducts.begin(); 
    ...
    try {
      ...
      while ((iPDGID != absMatchPDGIDs_.end()) && !isStatus2) {
  	isStatus2 = 
	  ((*iPDGID == GenTauDecayID::TAUPDGID) && 
	   (fabs(reco::GenParticleRef(pGenParticles, tauKey)->pdgId()) == GenTauDecayID::TAUPDGID) 
	   && iTau->isStatus2DecayProduct()) || //for taus
	  iTau->hasRightMother(*iPDGID); //for non-taus
  	++iPDGID;
      }
      if (isStatus2) {
