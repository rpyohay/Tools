			 ...
			 string savePath)
{
  ...
  for (map<string, pair<string, string> >::const_iterator iEffHist = effHistMap.begin(); 
    ...
    effGraph.SetName(effCanvasName.c_str());
				     ...
				     string savePath, const float weight)
{
  ...
  for (map<string, pair<string, string> >::const_iterator iEffHist = effHistMap.begin(); 
    ...
    TPad histPad(effCanvasName.c_str(), "", 0, 0, 1, 1);
    ...
    if (effCanvas != NULL) {
      effGraph = (TGraphAsymmErrors*)effCanvas->GetPrimitive(effCanvasName.c_str());
      if (effGraph == NULL) {
...
//make canvases with multiple graphs from different input files and save them to the output file
ErrorCode plotMultipleGraphsDifferentFiles(TFile& out, map<pair<string, string>, 
					   vector<pair<pair<TFile*, Option_t*>, 
					   pair<Color_t, Style_t> > > >& canvasMap, 
					   const map<string, vector<string> >& binLabelMap, 
					   string savePath)
{
  string fnName("ErrorCode plotMultipleGraphsDifferentFiles(TFile& out, map<pair<string, ");
  fnName+="string>, vector<pair<pair<TFile*, Option_t*>, pair<Color_t, Style_t> > > >& ";
  fnName+="canvasMap, const map<string, vector<string> >& binLabelMap, string savePath)";
  ...
  for (map<pair<string, string>, vector<pair<pair<TFile*, Option_t*>, 
    ...
    //get a pointer to the custom bin labels if applicable
    map<string, vector<string> >::const_iterator iGraphLabels = 
      binLabelMap.find(canvas1DName);
    ...
    Int_t canvasWidth = defaultCanvasWidth;
    Int_t canvasHeight = defaultCanvasHeight;
    if (iGraphLabels != binLabelMap.end()) {
      canvasWidth = 1200;
      canvasHeight = 600;
    }
    TCanvas canvas1D(canvas1DName.c_str(), "", canvasWidth, canvasHeight);
    setCanvasOptions(canvas1D, 1, 0, 0);
    if (iGraphLabels != binLabelMap.end()) canvas1D.cd()->SetRightMargin(0.1);

    //loop over vector of graphs and their options to put on one canvas
    for (vector<pair<pair<TFile*, Option_t*>, pair<Color_t, Style_t> > >::iterator iGraph = 
	   iCanvas->second.begin(); iGraph != iCanvas->second.end(); ++iGraph) {

      //unpack this vector
      TFile* in = iGraph->first.first;
      Option_t* drawOption = iGraph->first.second;
      Color_t markerColor = iGraph->second.first;
      Style_t markerStyle = iGraph->second.second;

      //get canvas from file
      TGraphAsymmErrors* graph = 
	getObjectFromCanvas<TGraphAsymmErrors>(*in, canvas1DName, canvas1DName, 0);
	...
	return CANNOT_RETRIEVE_OBJECT;
      ...
      Double_t xAxisLabelSize = defaultXAxisLabelSize;
      if (iGraphLabels != binLabelMap.end()) {
	setAxisLabels(graph->GetXaxis(), iGraphLabels->second);
	xAxisLabelSize = 0.04;
      }
      setGraphOptions(*graph, markerColor, 0.7, markerStyle, unit.c_str(), "");
...
void plotNiceDifferentFiles(map<pair<string, string>, vector<pair<pair<string, Option_t*>, 
			    pair<Color_t, Style_t> > > >& canvasMap, 
			    const map<string, vector<string> >& binLabelMap, 
			    const string& outputFileName, const string& savePath)
{
  ...
  /*create a map identical to canvasMap but with the file names replaced by corrrsponding TFile 
    pointers*/
  map<pair<string, string>, vector<pair<pair<TFile*, Option_t*>, 
    pair<Color_t, Style_t> > > > canvasMapWithFilePtrs;
  for (map<pair<string, string>, vector<pair<pair<string, Option_t*>, 
	 pair<Color_t, Style_t> > > >::const_iterator iCanvas = canvasMap.begin(); 
       iCanvas != canvasMap.end(); ++iCanvas) {
    vector<pair<pair<TFile*, Option_t*>, pair<Color_t, Style_t> > > graphMapWithFilePtrs;
    for (vector<pair<pair<string, Option_t*>, pair<Color_t, Style_t> > >::const_iterator iGraph = 
	   iCanvas->second.begin(); iGraph != iCanvas->second.end(); ++iGraph) {
      TFile* inputFile = new TFile(iGraph->first.first.c_str());
      graphMapWithFilePtrs.
	push_back(make_pair(make_pair(inputFile, iGraph->first.second), iGraph->second));
    }
    canvasMapWithFilePtrs[iCanvas->first] = graphMapWithFilePtrs;
  }

  //make graphs
  plotMultipleGraphsDifferentFiles(out, canvasMapWithFilePtrs, binLabelMap, savePath);
  ...
  out.Close();
  for (map<pair<string, string>, vector<pair<pair<TFile*, Option_t*>, 
	 pair<Color_t, Style_t> > > >::iterator iCanvas = canvasMapWithFilePtrs.begin(); 
       iCanvas != canvasMapWithFilePtrs.end(); ++iCanvas) {
    for (vector<pair<pair<TFile*, Option_t*>, pair<Color_t, Style_t> > >::iterator iGraph = 
	   iCanvas->second.begin(); iGraph != iCanvas->second.end(); ++iGraph) {
      iGraph->first.first->Close();
      delete iGraph->first.first;
    }
  }
}
...
/*create a histogram of (data - bkg.)/
  sqrt(jetFakeStatErr^2 + jetFakeSystErr^2)*/
			       ...
			       const TH1F* allEWBkg)
{
  ...
  for (Int_t iBin = 0; iBin <= (nomBkg->GetNbinsX() + 1); ++iBin) {
    const Double_t dataNomBkgStatErr = ratioHist->GetBinError(iBin);
    const Double_t bkgNom = nomBkg->GetBinContent(iBin);
    const Double_t bkgAllQCD = allQCDBkg->GetBinContent(iBin);
    const Double_t bkgAllEW = allEWBkg->GetBinContent(iBin);
    const Double_t statErr = nomBkg->GetBinError(iBin);
    const Double_t allQCDStatErr = allQCDBkg->GetBinError(iBin);
    const Double_t allEWStatErr = allEWBkg->GetBinError(iBin);
    const Double_t avg = (bkgNom + bkgAllQCD + bkgAllEW)/3.0;
    const Double_t avgStatErr = 
      sqrt(statErr*statErr + allQCDStatErr*allQCDStatErr + allEWStatErr*allEWStatErr)/3.0;
    Double_t rangeHigh = avg + avgStatErr;
    if ((bkgAllQCD + allQCDStatErr) > rangeHigh) rangeHigh = bkgAllQCD + allQCDStatErr;
    if ((bkgAllEW + allEWStatErr) > rangeHigh) rangeHigh = bkgAllEW + allEWStatErr;
    Double_t rangeLow = avg - avgStatErr;
    if ((bkgAllQCD - allQCDStatErr) < rangeLow) rangeLow = bkgAllQCD - allQCDStatErr;
    if ((bkgAllEW - allEWStatErr) < rangeLow) rangeLow = bkgAllEW - allEWStatErr;
    const Double_t systErrHigh = rangeHigh - avg;
    const Double_t systErrLow = avg - rangeLow;
    const Double_t systErr = systErrHigh > systErrLow ? systErrHigh : systErrLow;
    Double_t binContent = systErr == 0.0 ? 0.0 : (ratioHist->GetBinContent(iBin)/systErr);
    ratioHist->SetBinContent(iBin, binContent);
    ratioHist->SetBinError(iBin, dataNomBkgStatErr/statErr); //meaningless
  }
  ratioHist->GetYaxis()->SetTitle("#frac{Data - Bkg.}{Bkg. stat. + syst. error}");
  ratioHist->GetYaxis()->SetRangeUser(-3.0, 3.0);
  return ratioHist;
}
...
// 	  pHist->SetFillColor(0);
  ...
  for (vector<string>::const_iterator iCanvasName = canvasNames.begin(); 
	...
	if (i == 0) stackSumHist = stackHist;
      ...
      stackSumHist->SetLineColor(kRed);
      stackSumHist->SetFillColor(kRed);
      stackSumHist->SetFillStyle(3005);
      stackSumHist->Draw("e2same");
      string drawOpt("SAME");
...
//     cout << "sigmaB = " << setprecision(3) << sigmaB << endl;
  ...
  cout << "--------------------------------\n";
  cout << "B-->A normalization factor: " << setprecision(3) << norm << endl;
  cout << "--------------------------------\n";
		   ...
		   const bool ma9GeV)
{
  ...
  //print the raw number of events with mass >= 4 GeV in the region B data sample
  cout << "-----------------------------------\n";
  cout << "Raw no. events in region B data: " << nonIsoData->GetBinContent(5) << endl;
  cout << "-----------------------------------\n";
...
//   }
  ...
  /*create a final jet fake bkg. estimate averaging the region B data, region D data, and region B 
    MC predictions*/
  TH1F* avgBkg = (TH1F*)nonIsoData->Clone();
  avgBkg->SetName("muHadMassAvg");
  avgBkg->GetYaxis()->SetRangeUser(0.01, 100000.0);
  for (Int_t iBin = 0; iBin <= (avgBkg->GetNbinsX() + 1); ++iBin) {
    const Double_t bkgNom = avgBkg->GetBinContent(iBin);
    const Double_t bkgAllQCD = nonIsoWNonIsoData->GetBinContent(iBin);
    const Double_t bkgAllEW = nonIsoMCHist->GetBinContent(iBin);
    const Double_t statErr = avgBkg->GetBinError(iBin);
    const Double_t allQCDStatErr = nonIsoWNonIsoData->GetBinError(iBin);
    const Double_t allEWStatErr = nonIsoMCHist->GetBinError(iBin);
    const Double_t avg = (bkgNom + bkgAllQCD + bkgAllEW)/3.0;
    const Double_t avgStatErr = 
      sqrt(statErr*statErr + allQCDStatErr*allQCDStatErr + allEWStatErr*allEWStatErr)/3.0;
    avgBkg->SetBinContent(iBin, avg);
    avgBkg->SetBinError(iBin, avgStatErr);
    setHistogramOptions(avgBkg, kRed, 0.7, 20, 1.0, "m_{#mu+had} (GeV)", "");
    avgBkg->SetFillColor(kRed);
    avgBkg->SetFillStyle(3002);
  }

  //create a jet fake background histogram with full systematic error
  TH1F* bkgWithSystErr = (TH1F*)nonIsoData->Clone();
  bkgWithSystErr->SetName("muHadMassSystErr");
  bkgWithSystErr->GetYaxis()->SetRangeUser(0.01, 100000.0);
  for (Int_t iBin = 0; iBin <= (bkgWithSystErr->GetNbinsX() + 1); ++iBin) {
    const Double_t bkgNom = bkgWithSystErr->GetBinContent(iBin);
    const Double_t bkgAllQCD = nonIsoWNonIsoData->GetBinContent(iBin);
    const Double_t bkgAllEW = nonIsoMCHist->GetBinContent(iBin);
    const Double_t statErr = bkgWithSystErr->GetBinError(iBin);
    const Double_t allQCDStatErr = nonIsoWNonIsoData->GetBinError(iBin);
    const Double_t allEWStatErr = nonIsoMCHist->GetBinError(iBin);
    const Double_t avg = (bkgNom + bkgAllQCD + bkgAllEW)/3.0;
    const Double_t avgStatErr = 
    ...
    const Double_t systErrLow = avg - rangeLow;
    const Double_t systErr = systErrHigh > systErrLow ? systErrHigh : systErrLow;
    bkgWithSystErr->SetBinContent(iBin, avg);
    bkgWithSystErr->SetBinError(iBin, systErr);
    setHistogramOptions(bkgWithSystErr, kRed, 0.0, 20, 1.0, "m_{#mu+had} (GeV)", "");
    bkgWithSystErr->SetFillColor(kBlack);
    bkgWithSystErr->SetFillStyle(3001);

    if (iBin == 5) {
      cout << "----------------------\n";
      cout << "Stat. err. high = " << setprecision(3) << (systErrHigh/avg) << endl;
      cout << "Stat. err. low = " << setprecision(3) << (systErrLow/avg) << endl;
      cout << "----------------------\n";
    }
  }
...
//   totBkg->Add(resBkg, "HIST");
  totBkg->Add(/*nonIsoData*/avgBkg, "HIST");
  ...
  for (Int_t iHist = 0; iHist < totBkg->GetHists()->GetEntries(); ++iHist) {
	...
	if (hist->GetBinContent(iBin) != 0.0) {
      ...
      if (string(hist->GetName()) == /*"muHadMassDataControl"*/"muHadMassAvg") {
  ...
  cout << "Tot. bkg.: " << setprecision(3) << B << " +/- " << errSqrt << " (stat.)\n";
  cout << "Full error on tot. bkg.: " << setprecision(3) << bkgWithSystErr->GetBinError(5) << endl;
  ...
  cout << jetFakeBkgAroundJPsiErrSqrt << " (stat.)\n";
...
//   makeAndFormatPullHistogram(isoData, resBkg, nonIsoData, kAzure, "HIST");
  makeAndFormatPullHistogram(isoData, /*resBkg, *//*nonIsoData*/avgBkg, nonIsoWNonIsoData, 
			     nonIsoMCHist, kAzure, "HIST");
				       ...
				       const vector<Double_t>& newBinEdges = vector<Double_t>())
{
  ...
  if ((nNewBins > 0) && (newBinEdges.size() == ((unsigned int)nNewBins + 1))) {
